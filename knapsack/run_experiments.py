...
sched_arg_classic = {'step_size':10,
                     'gamma':0.1
                     }
diff_method_classic = "StepLR" # "StepLR", "SPOPlus"
diff_method_arg_classic = { 'n_samples':10, 
                            'sigma':1, 
                            'lambd':10,
                            'two_sides':False, 
                            'processes':1
...
time_limit_LD = 3600 #1h
batch_size_LD = 32
lr_LD = 0.001
model_shape_LD = [num_feat, 100, num_item]
dropout_LD = 0.2
schedulerType_LD = "OneCycleLR" # "StepLR", "ReduceLROnPlateau", "OneCycleLR", None
sched_arg_LD = {'step_size':50,
                     'gamma':0.5
                     }
diff_method_LD= "StepLR" # "StepLR", "SPOPlus"
diff_method_arg_LD = {  'max_lr':lr_LD,
                        'div_factor':10,
                        'final_div_factor':1e1,
                        'steps_per_epoch':num_data_train,
                        'epochs':epochs_LD
                        }

# Paramètre SG
epochs_SG = args.ep_sg
time_limit_SG = 3600 #1h
batch_size_SG = 32
lr_SG = 0.002
...
diff_method_SG= "StepLR" # "StepLR", "SPOPlus"
diff_method_arg_SG = { 'n_samples':10, 
                        'sigma':1, 
                        'lambd':10,
                        'two_sides':False, 
                        'processes':1
...
if epochs_LD > 0:
    ...
    wandbarg = {
            ...
            'project': "DFL_LD",
            'dir': "./",
            'name': f"LD_{dim}_{num_feat}_{num_item}_{num_data_train}",
            'group': f"{dim}_{num_feat}_{num_item}_{num_data_train}",
            'job_type': "LD",
            'config': {
...
if epochs_classic > 0:
    ...
    wandbarg = {
            ...
            'project': "DFL_LD",
            'dir': "./",
            'name': f"Classic_{dim}_{num_feat}_{num_item}_{num_data_train}",
            'group': f"{dim}_{num_feat}_{num_item}_{num_data_train}",
            'job_type': "Classic",
            'config': {
...
if epochs_SG > 0:
    ...
    wandbarg = {
            ...
            'project': "DFL_LD",
            'dir': "./",
            'name': f"SG_{dim}_{num_feat}_{num_item}_{num_data_train}",
            'group': f"{dim}_{num_feat}_{num_item}_{num_data_train}",
            'job_type': "SG",
            'config': {
